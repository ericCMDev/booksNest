...
struct BarCodeScanner : UIViewControllerRepresentable {
    ...
    func makeUIViewController (context: Context) -> UIViewController {
        ...
        metadataOutput.setMetadataObjectsDelegate(context.coordinator,queue:
            DispatchQueue.main)
            ...
            context.coordinator.previewLayer=AVCaptureVideoPreviewLayer(session:
                                                                            context.coordinator.captureSession)
    ...
    metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        if let metadataObject = metadataObjects.first {
            guard let readableObject = metadataObject as?
                    ...
                    print(code)
                    parent.isbn = code
                        BookSearchManager().getBookInfo(isbn: code) { booksFromBarCode in
                            DispatchQueue.main.async {
                                self.parent.foundBooks = booksFromBarCode
                            }
                        }
                    }
}
